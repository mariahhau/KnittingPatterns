...

    {
                    ...
                    if (row >= 0 && row < (double)rows.Value)
                    {
                            ...
                            { 
                                    if (stitchList[(int)row][(int)col] != brush.Color.ToArgb())
                                    {
                                            ...
                                            graph.FillRectangle(brush, (float)x + pen.Width, (float)y + pen.Width, cellSize - pen.Width, cellSize - pen.Width);
        ...
        private void stitches_ValueChanged(object sender, EventArgs e)
        {
            ...
            if (stitchCount < stitches.Value)
            {
                if (stitchList[0].Count < stitches.Value)
                {
                    ...
                    Region region = new Region(new Rectangle(stitchCount * cellSize , 0, canvasPanel.Width, canvasPanel.Height));
            ...
            else
            {
                ...
                Region region = new Region(new Rectangle((int)stitches.Value * cellSize + (int)pen.Width, 0, canvasPanel.Width, canvasPanel.Height));
                colNumberPanel.Invalidate(region, false);
                canvasPanel.Invalidate(region, false);
        ...
        private void rows_ValueChanged(object sender, EventArgs e)
        {
            ...
            if (rowCount < rows.Value)
            {
                ...
                Region region = new Region(new Rectangle(0,rowCount * cellSize, canvasPanel.Width, canvasPanel.Height));
            ...
            else
            {
                ...
                Region region = new Region(new Rectangle(0, (int)rows.Value * cellSize + (int)pen.Width, canvasPanel.Width, canvasPanel.Height));
                Region rowRegion = new Region(new Rectangle(0, (int)rows.Value * cellSize + gridY + (int)pen.Width, canvasPanel.Width, canvasPanel.Height));
                canvasPanel.Invalidate(region, false);
